...
class NumericGenerator(BaseGenerator):
    ...
    def _set_default_ranges(self) -> None:
        ...
        if (
            self.numeric_config.min_value is None
            or self.numeric_config.max_value is None
        ):
            defaults = {
    ...
    def generate(self) -> int | float | Decimal:
        """
        ...
        if self.numeric_config.subtype == NumericSubtype.INTEGER:
            ...
            raise ValueError(
                f"Unsupported numeric subtype: {self.numeric_config.subtype}"
            )
    ...
    def generate_with_distribution(
        self, distribution: str = "uniform", **kwargs
    ) -> int | float | Decimal:
        """
