...
class SchemaProcessor(BaseProcessor):
    ...
    def parse_schema(self) -> None:
        ...
        """
        self.logger.info(
            f"Parsing schema for dataset: {self.dataset_schema.table_name}"
        )
        ...
        try:
            ...
            self.logger.info(
                f"Created {len(self.generators)} generators for {len(self.dataset_schema.fields)} fields"
            )
            ...
            if missing_fields:
                self.logger.warning(
                    f"Failed to create generators for fields: {missing_fields}"
                )
    ...
    def process_dataset(self, record_count: int) -> list[dict[str, Any]]:
        ...
        self.logger.info(
            f"Generating {record_count} records for {self.dataset_schema.table_name}"
        )
        ...
        for field_def in self.dataset_schema.fields:
            ...
            if field_def.nullable and random.random() < field_def.null_probability:
                ...
                try:
                    ...
                    self.logger.error(
                        f"Error generating value for field '{field_name}': {e}"
                    )
                    record[field_name] = None
    ...
    def process_dataset_optimized(self, record_count: int) -> list[dict[str, Any]]:
        ...
        self.logger.info(
            f"Generating {record_count} records (optimized) for {self.dataset_schema.table_name}"
        )
        ...
        for field_def in self.dataset_schema.fields:
            ...
            try:
                ...
                if field_def.nullable and field_def.null_probability > 0:
                    values = self._apply_null_probability(
                        values, field_def.null_probability
                    )
                ...
                self.logger.error(
                    f"Error generating batch values for field '{field_name}': {e}"
                )
                field_values[field_name] = [None] * record_count
        ...
        for i in range(record_count):
            record = {
                field_name: values[i] for field_name, values in field_values.items()
            }
            records.append(record)
    ...
    def _apply_null_probability(
        self, values: list[Any], null_probability: float
    ) -> list[Any]:
        """
        ...
        """
        return [
            None if random.random() < null_probability else value for value in values
        ]
        ...
        try:
            ...
            if not is_valid:
                ...
                result["errors"].append(
                    f"Cannot create generator for field type: {field_def.type}"
                )
